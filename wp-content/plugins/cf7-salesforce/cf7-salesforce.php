<?php
/**
* Plugin Name: Contact Form 7 Salesforce
* Description: Integrates Contact Form 7 and <a href="https://wordpress.org/plugins/contact-form-entries/">Contact Form Entries Plugin</a> with Salesforce allowing form submissions to be automatically sent to your Salesforce account 
* Version: 1.1.5
...
self::$plugin->instance();
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo $id ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Salesforce Plugin','cf7-salesforce') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php _e('Dismiss Notice','cf7-salesforce') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice below-h2 is-dismissible" data-id="<?php echo $id ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Salesforce Plugin','cf7-salesforce') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
$value=trim(implode(' ',$value));
  ...
  */
  ...
  $desc= !empty($feed['error']) ? $feed['error'] : __("Unknown Error",'cf7-salesforce');
  }
  ...
  */
  ...
  printf('<input type="checkbox" name="'.$this->id.'_update" id="'.$this->id.'_update" value="1" /><label for="'.$this->id.'_update" title="%s">%s</label><br /><br />', __('Create or update this entry in Salesforce. The fields will be mapped according to the form feed settings.', 'cf7-salesforce'), __('Send to Salesforce', 'cf7-salesforce'));
  } else {
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Salesforce Logs','cf7-salesforce').'" href="'.$log_url.'">'.__('Salesforce Logs','cf7-salesforce').'</a>';
...
//  } 
  ...
  $log_link=' <a href="'.$log_url.'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','cf7-salesforce')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
