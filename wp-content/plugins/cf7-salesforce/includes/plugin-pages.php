...
$push=$this->push($entry_detail,$form,'add_note');
  ...
  ?>
  <div class="vx_row">
  <label><input type="checkbox" name="<?php echo $this->id ?>_send" value="yes"> <?php _e('Send to Salesforce','cf7-salesforce') ?></label>
  </div>
  <?php  
}
    /**
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . __('Settings', 'cf7-salesforce') . '</a>' );
        }
        return $links;
   }
  /**
...
$info_data=$api->handle_code();
 ...
 /* if(!empty($token)){
  ...
  //
  self::$tooltips = array(
 'vx_feed_name' =>  __('Enter feed name of your choice.', 'cf7-salesforce'),
  'vx_sel_object' => __('Select the Object to Create when a Form is Submitted.', 'cf7-salesforce'),
   'vx_sel_account' =>__('Select the Salesforce account you would like to export entries to.', 'cf7-salesforce'),
  'vx_sel_form' => __('Select the Contact Form you would like to integrate with Salesforce. Contacts generated by this form will be automatically added to your Salesforce account.', 'cf7-salesforce'),
  
  'vx_map_fields' => __('Associate your Salesforce fields to the appropriate Contact Form fields.', 'cf7-salesforce'),
  ...
  'vx_manual_export' => __('If you do not want all entries sent to Salesforce, but only specific, approved entries, check this box. To manually send an entry to Salesforce, go to Entries, choose the entry you would like to send to Salesforce, and then click the "Send to Salesforce" button.', 'cf7-salesforce'),
  
    'vx_entry_notes' => __('Enable this option if you want to synchronize Contact Form entry notes to Salesforce Object notes. For example , when you add a note to a Contact Form entry, it will be added to the Salesforce Object selected in the feed.', 'cf7-salesforce'),
    
      'vx_primary_key' => __('Which field should be used to update existing objects?', 'cf7-salesforce'),
...
<div class="table_head_i"><?php _e('Capsule', 'cf7-salesforce'); ?></div>  
<div class="vx_group">
...
<button class="button vx_send_crm_btn" data-crm="<?php echo $this->crm_name; ?>" type="button" data-action="send_to_crm_<?php echo $this->id ?>" value="yes">
<span class="reg_ok"><i class="fa fa-send"></i> Send to <?php echo $this->crm_name; ?></span> 
<span class="reg_proc" style="display: none;"><i class="fa fa-circle-o-notch fa-spin"></i> Sending ...</span>
</button>
  <?php
      if($comments ){
  ?>
  <a href="<?php echo $log_url ?>" target="_blank" class="button"><i class="fa fa-external-link"></i> Go to Logs</a>
  <?php
      }
  ?>
  </p>
...
if(isset($res['error'])){
  ...
  */
  ...
  $(this).text('<?php _e('Show Key','cf7-salesforce') ?>');  
  $(this).removeClass('vx_hidden');
  key.attr('type','password');  
  }else{
  $(this).text('<?php _e('Hide Key','cf7-salesforce') ?>');  
...
start_tooltip();  
  ...
  */
  ...
  _e('You do not have permissions to access this page','cf7-salesforce');    
  return;
  }
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = __('Salesforce','cf7-salesforce');
        return $plugins;
    }
  /**
...
$field_options="<option>".__("Select Object",'cf7-salesforce')."</option>"; 
  if(is_array($objects)){
...
include_once(self::$path . "templates/settings.php");
 ...
 $offset=$this->time_offset();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id."_edit_settings")){ 
  _e('You do not have permissions to save settings','cf7-salesforce');
  return;   
  }
...
$msgs=array();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  _e('You do not have permissions to edit/save feed','cf7-salesforce'); 
  return;
  }
  //
...
//  update_option($this->id.'_msgs',array());
  ...
  */
  ...
  ?>
  <i class="vx_icons vxc_tips fa fa-question-circle" data-tip="<?php echo self::$tooltips[$str] ?>"></i> 
  <?php  
  }
}
}
