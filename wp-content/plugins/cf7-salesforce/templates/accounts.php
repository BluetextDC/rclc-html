...
$(".sf_login").click(function(e){
    ...
    if(client_id!=new_id){
          e.preventDefault();   
     alert("<?php _e('Salesforce Client ID Changed.Please save new changes first','cf7-salesforce') ?>");   
    }    
    }
})
  ...
  if(!confirm('<?php _e('Notification - Remove Connection?','cf7-salesforce'); ?>')){
  e.preventDefault();   
  }
  });
...
<div class="vx_wrap">
  ...
  if(empty($id)){
  _e("Salesforce Accounts", 'cf7-salesforce');
  }else{
  _e("Salesforce Account #", 'cf7-salesforce'); echo $id;    
  } 
  if(empty($id) || $new_account_id != $id){
 ?> <a href="<?php echo $new_account ?>" class="add-new-h2" title="<?php _e('Add New Account','cf7-salesforce'); ?>"><?php _e('Add New Account','cf7-salesforce'); ?></a> 
 <?php
}if(!empty($id)){
 ?>
 <a href="<?php echo $page_link ?>" class="add-new-h2" title="<?php _e('Back to Accounts','cf7-salesforce'); ?>"><?php _e('Back to Accounts','cf7-salesforce'); ?></a>
 <?php
}
 ?>
  </h2>
  <p style="text-align: left;"> <?php echo sprintf(__("If you don't have a Salesforce account, you can %ssign up for one here%s.", 'cf7-salesforce'), "<a href='https://www.salesforce.com/' target='_blank' title='".__('Sign Up for Salesforce CRM','cf7-salesforce')."'>" , "</a>") ?> </p>
    <?php 
$this->show_msgs($msgs);               
