...
.vxcf_options_row .howto{
  ...
  ?>
  <div class="vx_wrap"> 
    <h2 class="vx_img_head"><img alt="<?php _e("Salesforce Feeds", 'cf7-salesforce') ?>" title="<?php _e("Salesforce Feeds", 'cf7-salesforce') ?>" src="<?php echo $this->get_base_url()?>images/salesforce-crm-logo.png?ver=1" /> <?php 
echo sprintf(__('Edit Salesforce Feed # %d','cf7-salesforce') ,$fid); 
  if(empty($id) || $new_account_id != $id){
 ?> <a href="<?php echo $new_feed_link ?>" class="add-new-h2"><?php _e("Add New Feed",'cf7-salesforce'); ?></a> 
 <?php
}
 ?>
 <a href="<?php echo $feeds_link ?>" class="add-new-h2"><?php _e("Back to Feeds",'cf7-salesforce'); ?></a>
...
if(empty($feed_name)){
  ...
  ?>
  <div class="error" id="message" style="margin-top:20px;"><?php echo wpautop(sprintf(__("We are unable to login to Salesforce with the provided API token. Please make sure they are valid in the %sSettings Page%s", 'cf7-salesforce'), "<a href='".$this->link_to_settings()."'>", "</a>")); ?></div>
  <?php
  // return;
  } 
  ?>
  ...
  <?php wp_nonce_field("vx_nonce") ?>
    <div class="vx_div">
      <div class="vx_head">
<div class="crm_head_div"> <?php _e('1. Enter feed name',  'cf7-salesforce' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_name" class="left_header"><?php _e("Feed Name", 'cf7-salesforce'); ?>
  ...
  <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php _e('2. Select Salesforce Account.', 'cf7-salesforce'); ?></div>
<div class="crm_btn_div" title="<?php _e('Expand / Collapse','cf7-salesforce') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_account" class="left_header"><?php _e("Salesforce Account", 'cf7-salesforce'); ?>
  <?php $this->tooltip("vx_sel_object") ?>
 </label>
  </div>
  <div class="vx_col2">
  <select id="vx_account" class="crm_sel load_object" name="account" autocomplete="off">
  <option value=""><?php _e("Select a Salesforce Account", 'cf7-salesforce'); ?></option>
  <?php
  ...
  <div class="clear"></div>
    ...
    <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php _e('3. Select Contact Form.', 'cf7-salesforce'); ?></div>
<div class="crm_btn_div" title="<?php _e('Expand / Collapse','cf7-salesforce') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_form_id" class="left_header"><?php _e("Contact Form Form", 'cf7-salesforce'); ?>
  <?php $this->tooltip("vx_sel_form") ?>
 </label>
  </div>
  <div class="vx_col2">
  <select id="vx_form_id" class="crm_sel load_object" name="form_id" autocomplete="off">
  <option value=""><?php _e("Select a Form", 'cf7-salesforce'); ?></option>
  <?php
  ...
  <div class="clear"></div>
    ...
    <div id="crm_ajax_div_object" style="display:none; text-align: center; line-height: 100px;"><i> <?php _e('Loading, Please Wait...','cf7-salesforce'); ?></i></div>
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php _e('This is a required field','cf7-salesforce') ?></div></div>');
    return false;
    }  
   })  
 }
  ...
  e.preventDefault(); 
    if(!confirm('<?php _e('Are you sure to remove ?','cf7-salesforce') ?>')){
      return;
  }
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php _e('Please Select Field Name','cf7-salesforce') ?>');
     return;   
    }  
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  $(document).on('click','.vxc_tips',function(e){
    ...
    jQuery('#crm_note_fields').select2({ placeholder: '<?php _e('Select Field','cf7-salesforce') ?>'});
} 
...
jQuery('#vx_add_fields_select').trigger('change');
  ...
  button_state_vx('ajax',button);
  vx_ajax=$.post(ajaxurl,{action:'get_field_map_<?php echo $this->id ?>',form_id:form_id,account:account,module:module,id:id,vx_crm_ajax:vx_crm_ajax,refresh:refresh},function(res){
  fields.html(res);   
...
var panel=$(this);
 ...
 // return;
  ...
  vx_ajax=$.post(ajaxurl,{action:'get_field_map_object_<?php echo $this->id ?>',form_id:form_id,account:account,id:id,vx_crm_ajax:vx_crm_ajax},function(res){
  fields.html(res);   
  //   primary.html(re.fields);
  fields.slideDown();   
start_tooltip();
...
vx_select();
  function vx_select(){
      $('#vx_add_fields_select').select2({ placeholder: '<?php _e('Select Field','cf7-salesforce') ?>'});
	  add_note_sel();
  }
